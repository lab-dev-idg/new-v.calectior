    ...
    <div className="w-full lg:w-72 flex flex-col gap-4 shrink-0">
      ...
      <div className="glass-card p-5 flex flex-col justify-between">
        <div>
          <h2 className="text-xs font-bold text-idg-gold uppercase tracking-widest mb-4 flex items-center gap-2">
            ...
            <span>{lang === 'ku' ? 'چاودێری زیرەکی سیستەم' : 'AI SYSTEM PULSE'}</span>
          </h2>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-white/5 text-center">
              <div>
                <span className="stat-label">{lang === 'ku' ? 'لێکدانی ئەلگۆریتم' : 'System Pulse'}</span>
              </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between items-center py-1.5 border-b border-white/5">
                <span className="opacity-60">{lang === 'ku' ? 'یاسای چالاک:' : 'Customs Legislation:'}</span>
                <span className="font-semibold text-idg-gold font-sans text-[10px]">{t.lawCaption.slice(0, 16)}...</span>
              </div>
              <div className="flex justify-between items-center py-1.5 border-b border-white/5">
                <span className="opacity-60">{lang === 'ku' ? 'کۆدی تێرمینال:' : 'Terminal Node:'}</span>
                <span className="font-mono font-bold">{PORTAL_SYSTEM_INFO.terminalCode}</span>
              </div>
              <div className="flex justify-between items-center py-1.5">
                <span className="opacity-60">{lang === 'ku' ? 'پاراستنی کاڵا:' : 'National Protection:'}</span>
      ...
      <div className="glass-card p-5">
        <h3 className="text-xs font-bold text-idg-gold uppercase tracking-widest mb-4 flex items-center gap-2">
          ...
          <span>{lang === 'ku' ? 'کورتەی کاڵاکان' : 'GATEWAY COMMODITIES'}</span>
        </h3>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center text-xs">
            <span className="flex items-center gap-1.5 text-emerald-400 font-semibold">
              ...
              {lang === 'ku' ? 'ڕێگەپێدراو' : 'Allowed'}
            </span>
            <span className="font-mono font-bold text-slate-300">{lists.allowed.length} items</span>
          </div>

          <div className="flex justify-between items-center text-xs">
            <span className="flex items-center gap-1.5 text-amber-400 font-semibold">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
              {lang === 'ku' ? 'پێویست بە مۆڵەت' : 'Restricted'}
            </span>
            <span className="font-mono font-bold text-slate-300">{lists.restricted.length} items</span>
          </div>

          <div className="flex justify-between items-center text-xs">
            <span className="flex items-center gap-1.5 text-rose-500 font-semibold">
              <span className="w-1.5 h-1.5 rounded-full bg-rose-500" />
              {lang === 'ku' ? 'قەدەغەکراو' : 'Prohibited'}
            </span>
            <span className="font-mono font-bold text-slate-300">{lists.prohibited.length} items</span>
          </div>
        </div>

        <div className="bg-white/5 p-3 rounded-xl border border-white/5 mt-4 text-center">
          <span className="stat-label text-[9px] block mb-0.5">{lang === 'ku' ? 'کۆی گشتی چاودێریکراو' : 'Total Monitored Items'}</span>
